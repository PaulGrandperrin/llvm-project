...
AliasResult ScopedNoAliasAAResult::alias(const MemoryLocation &LocA,
  ...
  if (LocA.AATags.NoAliasSideChannel && LocB.AATags.NoAliasSideChannel) {
    if (noAliasByIntrinsic(ANoAlias, LocA.AATags.NoAliasSideChannel, BNoAlias,
                           LocB.AATags.NoAliasSideChannel, nullptr, nullptr,
                           AAQI))
      return NoAlias;

    if (noAliasByIntrinsic(BNoAlias, LocB.AATags.NoAliasSideChannel, ANoAlias,
                           LocA.AATags.NoAliasSideChannel, nullptr, nullptr,
                           AAQI))
      return NoAlias;
  }
...
ModRefInfo ScopedNoAliasAAResult::getModRefInfo(const CallBase *Call,
  ...
  const MDNode *CSScopes =
    Call->getMetadata(LLVMContext::MD_alias_scope);
  if (!mayAliasInScopes(CSScopes, Loc.AATags.NoAlias))
    return ModRefInfo::NoModRef;

  if (Loc.AATags.NoAliasSideChannel) {
    if (noAliasByIntrinsic(Loc.AATags.NoAlias, Loc.AATags.NoAliasSideChannel,
                           CSNoAlias, nullptr, nullptr, Call, AAQI))
      return ModRefInfo::NoModRef;

    if (noAliasByIntrinsic(CSNoAlias, nullptr, Loc.AATags.NoAlias,
                           Loc.AATags.NoAliasSideChannel, Call, nullptr, AAQI))
      return ModRefInfo::NoModRef;
  }
    ...
    return !mayAliasInScopes(
               dyn_cast<MDNode>(
                   cast<MetadataAsValue>(
                       A->getOperand(Intrinsic::SideNoAliasScopeArg))
                       ->getMetadata()),
               ANoAlias) &&
           !mayAliasInScopes(
               dyn_cast<MDNode>(
...
bool ScopedNoAliasAAResult::noAliasByIntrinsic(
  ...
  if (CallA) {
    ...
    // if the callsite (CallA) is derived from some noalias call(s) and the other
    // thing is not derived from those noalias call(s). This can be determined
    // only if CallA only accesses memory through its arguments.
  ...
  // We need to check now if any compatible llvm.side.noalias call is
...

  {
    ...
    for (Instruction *CA : CompatibleSet) {
      ...
      for (Instruction *CB : BNoAliasCalls) {
        ...
        // With the llvm.side.noalias version, we have different parts that can
        // represent a P:
        ...
        // - different objectId ?
        {
          ...
          auto ObjIdA =
              cast<ConstantInt>(
                  CA->getOperand(Intrinsic::SideNoAliasIdentifyPObjIdArg))
                  ->getZExtValue();
          auto ObjIdB =
              cast<ConstantInt>(
                  CB->getOperand(Intrinsic::SideNoAliasIdentifyPObjIdArg))
                  ->getZExtValue();
          if (ObjIdA != ObjIdB) {
        ...
        // Different Scope ? (except for unknown scope)
        {
          ...
          auto *CASnaScope = CA->getOperand(Intrinsic::SideNoAliasScopeArg);
          auto *CBSnaScope = CB->getOperand(Intrinsic::SideNoAliasScopeArg);
        ...
        // Different 'P' ?
        {
          Value *P_A = CA->getOperand(Intrinsic::SideNoAliasIdentifyPArg);
          Value *P_B = CB->getOperand(Intrinsic::SideNoAliasIdentifyPArg);
          ...
          Constant *CP_A = dyn_cast<Constant>(P_A);
          if (CP_A) {
            Constant *CP_B = dyn_cast<Constant>(P_B);
            if (CP_B) {
          ...
          // The noalias_sidechannel is not handled in the
          // Instruction::getAAMetadata for intrinsics
          P_A_Metadata.NoAliasSideChannel =
              CA->getOperand(Intrinsic::SideNoAliasIdentifyPSideChannelArg);
          P_B_Metadata.NoAliasSideChannel =
              CB->getOperand(Intrinsic::SideNoAliasIdentifyPSideChannelArg);
  ...
  for (auto &C : CompatibleSet)
    CompatibleSetMVs.push_back(
        cast<MetadataAsValue>(C->getOperand(Intrinsic::SideNoAliasScopeArg)));
    ...
    if (MRB != FMRB_OnlyAccessesArgumentPointees &&
      ...
      for (auto &C : CompatibleSet)
        if (PointerMayBeCapturedBefore(C, /* ReturnCaptures */false,
                                          /* StoreCaptures */false,
                                          CallB, DT)) {
          LLVM_DEBUG(dbgs() << "SNA: CSB: Pointer " << *C <<
                          " might be captured!\n");
          return false;
        }
    }
