...
LoadInst *LoadInst::cloneImpl() const {
  LoadInst *Result =
      new LoadInst(getType(), getOperand(0), Twine(), isVolatile(),
                   MaybeAlign(getAlignment()), getOrdering(), getSyncScopeID());
  // - we must keep the same number of arguments (for vector optimizations)
  // - if we duplicate the side channel, we can get into problems with passes
  ...
  if (hasNoaliasSideChannelOperand())
    Result->setNoaliasSideChannelOperand(
        UndefValue::get(getNoaliasSideChannelOperand()->getType()));
  return Result;
}

StoreInst *StoreInst::cloneImpl() const {
  StoreInst *Result = new StoreInst(getOperand(0), getOperand(1), isVolatile(),
