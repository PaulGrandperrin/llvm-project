...
bool LoopRotate::rotateLoop(Loop *L, bool SimplifiedLatch) {
  ...
  {
    SmallVector<Instruction*, 6> SideNoAliasOrNoAliasToDisconnect;
            ...
            if (UI->getParent() == OrigHeader) {
              ...
              if (PHINode *PUI = dyn_cast<PHINode>(UI)) {
                if (PUI->getNumIncomingValues() > 1) {
                  LLVM_DEBUG(llvm::dbgs() << "LoopRotation: NOT rotating - " <<
                             *II << "\n    used in PHI node " << *PUI <<
                             ",\n   in exit block with multiple entries.\n");
                  return false;
                }
              }
              SideNoAliasOrNoAliasToDisconnect.push_back(UI);
            }
          }

          if (UsedInsideHeader && UsedOutsideHeaderInsideLoop) {
            LLVM_DEBUG(llvm::dbgs() << "LoopRotation: NOT rotating - " <<
              *II << " used in header and other parts of the loop.\n"
    ...
    // usage
    for (Instruction *I : SideNoAliasOrNoAliasToDisconnect) {
      unsigned OpN;
      if (PHINode* PI = dyn_cast<PHINode>(I)) {
        ...
        if (II->getIntrinsicID() == Intrinsic::side_noalias) {
          OpN = Intrinsic::SideNoAliasNoAliasDeclArg;
  ...
  if (!NoAliasDeclInstructions.empty()) {
    ...
    //
    // with D: llvm.noalias.decl, U: side_noalias, depending on D
    ...
    // This ensures a safe fallback to 'may_alias' introduced by the rotate, as
    // U'' and U'
    // scopes will not be compatible wrt to the local restrict

    // Clone the NoAliasDecl again, insert right  after the original, move the
    // original to the NewHeader.

    Instruction* NewHeaderInsertionPoint = &(*NewHeader->begin());
    for (Instruction* NAD : NoAliasDeclInstructions) {
      ...
      auto tmp = ValueMap[NAD]; // use a local copy to avoid undefined behavior
