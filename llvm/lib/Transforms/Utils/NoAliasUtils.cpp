  ...
  for (Instruction &I : llvm::instructions(*F)) {
    if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(&I)) {
      ...
      case Intrinsic::side_noalias: {
        TrackIfIsUnknownFunctionScope(II, Intrinsic::SideNoAliasScopeArg);
        break;
      }
  ...
  for (auto *II : InterestingNoalias) {
    ...
    unsigned Index =
        (II->getIntrinsicID() == Intrinsic::noalias
             ? 0
             : (II->getIntrinsicID() == Intrinsic::side_noalias ? 1 : 2));
    ...
    const int ObjIdArg[] = {Intrinsic::NoAliasIdentifyPObjIdArg,
                            Intrinsic::SideNoAliasIdentifyPObjIdArg, -1};
    ...
    auto UA = dyn_cast<AllocaInst>(UO[0]);
    if (UA) {
