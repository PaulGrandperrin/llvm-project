...
TEST_F(IRBuilderTest, SideChannel) {
  ...
  EXPECT_TRUE(!L->hasNoaliasSideChannelOperand());

  auto *S = Builder.CreateStore(L, GV);
  EXPECT_TRUE(!S->hasNoaliasSideChannelOperand());

  L->setNoaliasSideChannelOperand(GV);
  EXPECT_TRUE(L->hasNoaliasSideChannelOperand());

  S->setNoaliasSideChannelOperand(GV);
  EXPECT_TRUE(S->hasNoaliasSideChannelOperand());

  EXPECT_EQ(L->getNoaliasSideChannelOperand(), GV);
  EXPECT_EQ(S->getNoaliasSideChannelOperand(), GV);

  L->removeNoaliasSideChannelOperand();
  EXPECT_TRUE(!L->hasNoaliasSideChannelOperand());

  S->removeNoaliasSideChannelOperand();
  EXPECT_TRUE(!S->hasNoaliasSideChannelOperand());
}
